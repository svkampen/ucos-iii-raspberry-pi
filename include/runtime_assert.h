...
#define WARN_IF_NOT(x) if (!(x)) { CPU_SR_ALLOC(); OS_CRITICAL_ENTER(); printf("Warning: in %s:%d: `!(%s)'\n", __FILE__, __LINE__, #x); OS_CRITICAL_EXIT(); }
...
#define ASSERT(x) if (!(x)) { __asm__("cpsid if"); printf("ASSERTION FAILED in %s:%d: `%s'\n", __FILE__, __LINE__, #x); hang(); }
